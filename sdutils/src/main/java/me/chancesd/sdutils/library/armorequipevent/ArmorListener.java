...
class ArmorListener implements Listener{
	...
	public void onInteract(final PlayerInteractEvent event) {
		...
		if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
			...
			final ArmorType oldArmorType = ArmorType.matchType(oldArmorPiece);

			// Checks if old armor is empty/air
			if (isEmpty(oldArmorPiece) || newArmorType.equals(oldArmorType)) {
				final ArmorEquipEvent armorEquipEvent = new ArmorEquipEvent(player, ArmorEquipEvent.EquipMethod.HOTBAR, newArmorType, oldArmorPiece, event.getItem());
				Bukkit.getServer().getPluginManager().callEvent(armorEquipEvent);
	...
	static boolean isEmpty(final ItemStack item) {
...
//	}
	...
	public void onBreak(final PlayerItemBreakEvent event){
		...
		if(type != null){
			...
			final ArmorEquipEvent armorEquipEvent = new ArmorEquipEvent(p, ArmorEquipEvent.EquipMethod.BROKE, type, event.getBrokenItem(), null);
			Bukkit.getServer().getPluginManager().callEvent(armorEquipEvent);
			if(armorEquipEvent.isCancelled()){
