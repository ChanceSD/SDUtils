package me.chancesd.sdutils.metrics;
...
public class Metrics {
	 ...
	 */
	public Metrics(final JavaPlugin plugin, final int serviceId, final boolean optOut) {
		this.plugin = plugin;
		// Get the config file
		...
		// Load the data
		final boolean enabled = !optOut || config.getBoolean("enabled", true);
	...
	public static class MetricsBase {
		...
		public MetricsBase(final String platform, final String serverUuid, final int serviceId, final boolean enabled,
		        ...
		        final Consumer<Runnable> submitTaskConsumer, final Supplier<Boolean> checkServiceEnabledSupplier, final BiConsumer<String, Throwable> errorLogger,
		        final Consumer<String> infoLogger, final boolean logErrors, final boolean logSentData, final boolean logResponseStatusText) {
		...
		private void submitData() {
			...
			final JsonObjectBuilder.JsonObject[] chartData = customCharts.stream().map(customChart -> customChart.getRequestJsonObject(errorLogger, logErrors))
	...
	public static class JsonObjectBuilder {
		...
		public JsonObjectBuilder appendField(final String key, final String[] values) {
			...
			final String escapedValues = Arrays.stream(values).map(value -> "\"" + escape(value) + "\"").collect(Collectors.joining(","));
